...
Ext.define('CustomApp', {
  ...
  processFeatures: function (features) {
    ...
    features = _.filter(features, function (f) { return f.raw.Release && _.contains(releases, f.raw.Release.ObjectID); });
  ...
  processReleases: function (releases) {
    var me = this;
    releases = _.filter(releases, function (release) { return me.selectedReleases[release.get('ObjectID')]; });

    this.releases = _(releases)
      .filter(function (release) { return me.selectedReleases[release.get('ObjectID')]; })
